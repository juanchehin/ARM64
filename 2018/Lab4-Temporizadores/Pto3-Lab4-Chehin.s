...
3) Utilizando los botones 1 y 2 modificar la intensidad del LED Rojo,
utilizando PWM para modificar el ancho del pulso activo.

• B1 sube intensidad LED Rojo.
• B2 baja intensidad LED Rojo.
...
// Recursos utilizados por el BOTON1
    .equ BOT_1_PORT,    4
    .equ BOT_1_PIN,     8
    .equ BOT_1_BIT,     12
    .equ BOT_1_MASK,    (1 << BOT_1_BIT)

// Recursos utilizados por el BOTON2
    .equ BOT_2_PORT,    4
    .equ BOT_2_PIN,     9
    .equ BOT_2_BIT,     13
...
// Recursos utilizados por la botonera
    .equ BOT_GPIO,      5
    .equ BOT_MASK,      ( BOT_1_MASK | BOT_2_MASK )
   ...
   // Apaga todos los bits gpio de los leds
    ...
    // Prescaler de 0 para que no parpadee
        MOV R0,#1000
        STR R0,[R1,#PR]

    // El valor del periodo para  Hz
        LDR R0,=1000
        STR R0,[R1,#MR3]
...
refrescar:
    ...
    LDR   R7,[R4,#(BOT_GPIO << 2)]

    // Verifica el estado del bit correspondiente a la tecla uno para aumentar intensidad
    ANDS  R1,R7,#(1 << BOT_1_BIT)
    // Si la tecla no esta apretada
    IT    NE
    BNE   aumentar

    // Verifica el estado del bit correspondiente a la tecla dos disminuir intensidad
    ANDS  R1,R7,#(1 << BOT_2_BIT)
    // Si la tecla no esta apretada
    IT    NE
    BNE   disminuir
