...
reset:
    ...
    // Configura los pines del display punto como gpio sin pull-up
    MOV R0,#(SCU_MODE_INACT | SCU_MODE_INBUFF_EN | SCU_MODE_ZIF_DIS | SCU_MODE_FUNC4)
    STR R0,[R1,#(PUNTO_PORT << 7 | PUNTO_PIN << 2)]
    ...
    //Configura los bits del punto como salida
    LDR R1,=GPIO_DIR0
...
//Carga del valor al display
    ...
    UBFX R0,R5,#16,#4                  //Activar punto
    CMP R0,#1
    ITT EQ
    MOVEQ R2,(PUNTO_MASK)
    STREQ R2,[R1,#(BOTON_GPIO << 2)]

    BL retardo

    MOV R2,(1<<16)                     //Desactivar punto
    STR R2,[R6,#(BOTON_GPIO<<2)]

    MOV R2, #0xFF
    UBFX R0,R5,#4,#4
    STR R2,[R6]
    MOV R2,#0b10
    STRB R2,[R1] //display 2 enable
    STR R3,[R6,#(LED_SEGM_GPIO<<2)]
    LDRB R8,[R9,R0]
    STRB R8,[R1,#(LED_SEGM_GPIO<<2)]
    ...
    MOV R2,(1<<16)
    STR R2,[R6,#(BOTON_GPIO<<2)]      //Desactivar punto

    UBFX R0,R5,#8,#4
    MOV R2,#0xFF
    STR R2,[R6]
    MOV R2,#0b100
    STRB R2,[R1] //display 3 enable
    STR R3,[R6,#(LED_SEGM_GPIO<<2)]
    LDRB R8,[R9,R0]
    STRB R8,[R1,#(LED_SEGM_GPIO<<2)]
