...
//Recursos del boton 3
.equ BUTTON3_PORT, 4
.equ BUTTON3_PIN, 10
.equ BUTTON3_BIT, 14
.equ BUTTON3_MASK, (1<<BUTTON3_BIT)

//Recursos del boton 4
.equ BUTTON4_PORT, 6
.equ BUTTON4_PIN, 7
.equ BUTTON4_BIT, 15
.equ BUTTON4_MASK, (1<<BUTTON4_BIT)

//Recursos de los botones
.equ BUTTON_GPIO, 5
.equ BUTTON_MASK, (BUTTON1_MASK|BUTTON2_MASK|BUTTON3_MASK|BUTTON4_MASK)
...
reset:
    ...
    STR R0,[R1,#(BUTTON3_PORT << 7 | BUTTON3_PIN << 2)]
    STR R0,[R1,#(BUTTON4_PORT << 7 | BUTTON4_PIN << 2)]
    ...
    MOV R11,#4 //Opcion de incrementado 4:numero de 4 cifras, 3:Unidad de mil, 2:Centena, 1:Decena, 0:unidad
...
entonces2:
    ...
    MOVEQ R11,#4
inc:
//Logica de incremento/decremento
    CMP R11,#4
    BNE inicio_ind
modo4:
...
inicio_ind:
    ...
    BNE segundo
primero:
    UBFX R2,R3,#0,#4 //extraigo primer digito
    BL ind
    BFI R3,R2,#0,#4  //reinserto el digito
    B finind
segundo:
    CMP R11,#1
    BNE tercero

    UBFX R2,R3,#4,#4 //extraigo segundo digito
    BL ind
    BFI R3,R2,#4,#4 //reinserto el digito
    B finind

tercero:
    CMP R11,#2
    BNE cuarto
    UBFX R2,R3,#8,#4 //extraigo tercer digito
    BL ind
    BFI R3,R2,#8,#4 //reinserto el digito
    B finind

cuarto:
    CMP R11,#3
    BNE finind
    UBFX R2,R3,#12,#4 //extraigo cuarto digito
    BL ind
    BFI R3,R2,#12,#4 //reinserto el digito
...
redisplay:
    ...
    LDR R0,=DP_MASK
    STR R0,[R7,#(DP_GPIO << 2)] //desactivar el dp
    CMP R4,R11 //comparo el digito actual con el modo de incremento
    IT EQ
    STREQ R0,[R6,#(DP_GPIO <<2)] //activa el dp

    //Paso al siguiente display
    ADD R4,#1
    CMP R4,#4
    IT EQ
    MOVEQ R4,#0
