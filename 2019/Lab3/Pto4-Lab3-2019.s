/* Chehin Juan Martin - FACET UNT - Pto4Lab3 - 2019
-------------------------------------------

4) Modifique el programa del ejercicio anterior para convertir el reloj en un cronometro. Para ello
se utilizará el boton aceptar para iniciar o detener la cuenta y el botón cancelar para volver a
cero.
...
*/
 ...
 .cpu cortex-m4          // Indica el procesador de destino
    .syntax unified         // Habilita las instarucciones Thumb-2
...
// Recursos utilizados por el boton ACEPTAR - P3_1 4:GPIO5[8]
    .equ BOTONA_PORT,   3
    .equ BOTONA_PIN,    1
    .equ BOTONA_BIT,    8
    .equ BOTONA_MASK,   (1 << BOTONA_BIT)

// Recursos utilizados por el boton CANCELAR - P3_2 4:GPIO5[9]
    .equ BOTONC_PORT,   3
    .equ BOTONC_PIN,    2
    .equ BOTONC_BIT,    9
    .equ BOTONC_MASK,   (1 << BOTONC_BIT)


// Recursos utilizados por los botones
    .equ BOTON_GPIO,      5
    .equ BOTON_MASK,      ( BOTON1_MASK | BOTON2_MASK | BOTON3_MASK | BOTON4_MASK | BOTONA_MASK | BOTONC_MASK )
...
espera:      .zero 2        // Variable el tiempo de espera
contador:    .hword 0x00    //guarda el numero a mostrar
...
reset:
  ...
  // Configura los pines de los display como gpio sin pull-up
    ...
    STR R0,[R1,#(BOTONA_PORT << 7 | BOTONA_PIN << 2)]
    STR R0,[R1,#(BOTONC_PORT << 7 | BOTONC_PIN << 2)]
 ...
 refrescar:
    ...
    ANDS  R1,R0,#(1 << BOTONA_BIT)     //Si la tecla esta apretada
    ...
    ANDS  R1,R0,#(1 << BOTONC_BIT)//   Si la tecla esta apretada
    MVN r5,r5
    ANDS r5,r1
    IT    NE
...
systick_init:
    ...
    LDR R1,=SYST_RVR
    LDR R0,=#(480000 - 1)     // Al modificar este valor el reloj cuenta mas o menos rapido
...
multiplexar:
    ...
    CMP R1,#4               // comparo si llego al 4 cuarto digito
    ITE EQ
